"""Individual cache optimization ablation on the race-fixed 16-slot baseline."""
...
def configs():
    ...
    base = next(c for c in original_configs() if c["full_slots"] == 16)
    return [{**base, "label": variant, "ablation": variant, "pre_optimization": False}
...
def summarize(root):
    ...
    output = {}
    lines = ["# Individual cache optimization ablation", "",
             ...
             "LRU; graphs and overlap disabled. Five repetitions; reverse variant order on odd repetitions. Each variant changes only one optimization. Race fixes retained in all variants.", "",
             ...
             "Fresh baseline versus logging-only, singleton-only and restore-nozero-only variants. No quality evaluation; no pooling with earlier studies.", ""]
...
async def experiment(root):
    design = configs()
    sources = [Path(__file__), HERE / "spark_run.py", HERE / "server.py", HERE / "instrumentation.py", HERE / "allocation_native.py", HERE / "individual_variant.py"]
    sources += [ROOT / "python/sglang/srt" / p for p in (
        "mem_cache/rsvd_eigh.py", "server_args.py", "mem_cache/memory_pool.py", "mem_cache/mamba_allocation.py", "mem_cache/mamba_radix_cache.py",
    ...
    save(root / "protocol.json", dict(
        ...
        groups=64, rounds=3, prefix=2048, suffix=16, output=128, seed=20261010,
        ordering="Same trace/seed within each repetition; reverse variant order on odd repetitions.",
        policy="lru", native_allocation=True, staging_full_states=16,
        note="Independent optimizations against a fresh race-fixed baseline. Pilots excluded; abort on failed validation, no retries.",
        hashes={str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest() for p in sources},
    ))
    completed = 0
    for phase in ("pilot", "full"):
        (root / phase).mkdir()
        for sequence, rep in enumerate(range(1 if phase == "pilot" else 5)):
            # Reverse variant order on alternate repetitions.
            ...
            for config in ordered:
                args = Namespace(groups=8 if phase == "pilot" else 64, rounds=2 if phase == "pilot" else 3,
                                 prefix=2048, output=128, seed=20261009 if phase == "pilot" else 20261010,
