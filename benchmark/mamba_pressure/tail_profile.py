...
def install():
    ...
    wrap(Req, 'init_next_round_input', 'cache_lookup', lambda req, *a, **k: {'rid': req.rid})
    wrap(MambaRadixCache, '_decompress_from_pool', 'restore',
         lambda tree, node, *a, **k: {'compressed': bool(node.mamba_compressed), 'node_id': node.id}, gpu=True)
    wrap(MambaRadixCache, 'drain_compression_completions', 'commit_loop', lambda *a, **k: {}, gpu=True)
    wrap(MambaRadixCache, '_process_compression_batch', 'svd',
         lambda tree, batch: {'items': len(batch)}, gpu=True, svd=True)
    wrap(Scheduler, 'run_batch', 'forward',
         lambda scheduler, batch, *a, **k: {'rids': [r.rid for r in batch.reqs], 'mode': str(batch.forward_mode)}, gpu=True)
    ...
    def info(scheduler, request):
        ...
        result.internal_state['cache_observations']['tail_profile_pending_spans'] = len(pending)
        return result
